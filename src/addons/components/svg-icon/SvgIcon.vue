<template>
    <svg @click="$emit('click')" :class=computed_class :style=computed_style :id="'svg_'+identifier" :width=computed_width :height=computed_height xmlns="http://www.w3.org/2000/svg"  :viewBox=computed_viewbox :fill=computed_fill :stroke=computed_stroke>
    </svg>
</template>
<script>
...
export default {
    name: "svg-icon",
    props: ['width', 'height','fill','stroke','icon','viewbox','x_class','x_style'],
    setup() {
    ...
    methods : {
        load_svg_content : function () {
            ...
            document.getElementById('svg_'+this.identifier).innerHTML = svgs[this.$props.icon];
        }
    },
    watch : {
        icon : {
